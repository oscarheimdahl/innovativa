...
function sortResult(paramsss, roomssss) {
    let params = {
        body: 1,
        temperature: 24,
        light: 0.4,
    ...
    Object.entries(rooms).forEach(room => {
        ...
        Object.entries(room[1]).forEach(attribute => {
            roomScore += compareType(attribute[0],params[attribute[0]],attribute[1]);
        });
...
function sortByScore(results, sortedResults, numberOfRooms) {
    ...
    for (i = 0; i < numberOfRooms; i++) {
        ...
        Object.entries(results).forEach(room => {
            ...
            if (room[1].roomScore < lowestScore && !sortedResults.some(roomResult => roomResult.roomScore === room[1].roomScore)) {
                lowestScoreRoom = {
                    roomName: room[0],
